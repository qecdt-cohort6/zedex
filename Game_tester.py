...
def choose_qubits(n=5,arq=ibmq_16_melbourne):
    ...
    if len(arq["qubits"]) < n:
        ...
        qubits = []
        start = str(random.randint(0,len(arq["qubits"])-1))
        qubits.append(start)
...
for item in Circuits:
	...
	arqq = arq(qubits=choose_qubits(qubit_num))

	arqq = {'connections':arqq['connections']}
	...
	html = d3_no_jup.draw(graph,arqq,qasm)
